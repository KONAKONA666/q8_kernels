...
from q8_gemm import q8_mm_bias
...
o_orig = torch.matmul(x.to(torch.float8_e4m3fn).half(), w.to(torch.float8_e4m3fn).half().t()) + bias[None, None, :].half()# nn.functional.linear(x, w, bias=None)
...
o_fp8_orig = torch._scaled_mm(x[1].to(torch.float8_e4m3fn),  w.to(torch.float8_e4m3fn).contiguous().t(), scale_a=torch.tensor([1.0]).cuda(), scale_b=torch.tensor([1.0]).cuda(), bias=bias.half())
...
for _ in range(5):
    o_q8_h = q8_mm_bias(x_quant_h, w_quant_h, bias, x_scales_h, w_scales_h, True)
...
for _ in range(5):
    o_fp8_orig = torch._scaled_mm(x_hadamard[0],  _w, scale_a=_scale, scale_b=_scale, bias=bias_half)
...
for _ in range(N_OUTER_ROUNDS):
    ...
    for i in range(N_ROUNDS):
        start_events[i].record()
        o_fp8_orig = torch._scaled_mm(x_hadamard[0],  _w, scale_a=torch.tensor([1.0]).cuda(), scale_b=torch.tensor([1.0]).cuda(), bias=bias_half)
        end_events[i].record()
    torch.cuda.synchronize()
    elapsed_times = [s.elapsed_time(e) for s, e in zip(start_events, end_events)]
    fp8_tflops.append((TFLOPS/2 * 1e-12)/(min(elapsed_times) * 1e-3))
print("FP8 TFLOPS: ", max(fp8_tflops))
